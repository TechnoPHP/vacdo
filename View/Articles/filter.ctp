				...
				<artical>
					<div class="artical_box">
						<div class="artical_thum">
							...
							echo $this->Html->image($post['Articlecoverimage']['namemedium'], array('class'=>'img-responsive'));
							}else{
						...
						<div class="artical_content">
							<P><?php echo $this->Html->link($this->Text->truncate($post['Article']['title'],70),array("controller"=>"posts","action"=>"view",'slug' => Inflector::slug($post['Article']['title'],'-'),'id'=>$post['Article']['id'])); ?></P>
							<div class="artical_thum_date"> 
								<?php echo $this->Time->nicedate($post['Article']['modified']); ?> 
							</div>
							<div class="comment">
								...
								<?php echo (count($post['Article']['postview_count'])>1)?$post['Article']['comment_count']." Comments":$post['Article']['postview_count']." Postviews" ;?>
							</div>
							<div class="star">
								<?php echo $this->Ratings->display_for($post); ?>
							</div>
							<?php echo $this->Html->link("Read More",array("controller"=>"posts","action"=>"view",'slug' => Inflector::slug($post['Article']['title'],'-'),'id'=>$post['Article']['id']),array("class"=>"read_more")); ?></a> 
						</div><!-- artical content -->
					</div><!-- artical_box -->
				</artical>
			</div>
