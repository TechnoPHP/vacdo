<?php echo $this->Html->meta('canonical', Router::url('/', true).'post/'.Inflector::slug(strtolower($post['Post']['title']),'-'),array('rel'=>'canonical', 'type'=>null, 'title'=>null, 'inline' => false)); ?>
<div class="contain">
<div class="row">
	<div class="col-md-3">
		<aside>
		...
		<div class="content_box">
			<div class="articles-details-header">
				<div class="articles-details-header-img"> 
					<?php if(!empty($post['Postcoverimage']['name'])){	
					echo $this->Html->image($post['Postcoverimage']['name'], array('class'=>'img-responsive'));
					echo $this->Html->meta(array('property'=>'og:image','type'=>'meta', 'content'=>substr(Router::url('/', true), 0, -1).'post/'.$post['Postcoverimage']['name'], 'rel' => null),NULL, array('inline'=>false));
					...
					echo $this->Html->meta(array('property'=>'og:url','type'=>'meta', 'content'=>Router::url('/', true).$post['Post']['title'], 'rel' => null),NULL, array('inline'=>false));
					?> 
				</div>
				<?php $services = array(
			...
			<div class="breadcrumblist">
				...
				echo '<li>';
				echo $this->Html->link('All', array('controller'=>'posts','action'=>'filter','admin'=>false));
				echo '</li>';
				foreach ($treePath as $key => $treeCategory) {
					...
					if ($total === $key + 1) {
						...
						echo $this->Html->link($treeCategory['Postcategory']['name'], array('controller'=>'posts','action'=>'filter','category' =>Inflector::slug($treeCategory['Postcategory']['name'],'-'), 'postcategoryid' => $treeCategory['Postcategory']['id'],'admin'=>false));
					}
					echo '</li>';
				}
				echo '</ul>';
				...
				<div class="gray_strip">
					<div class="col-md-12">
						...
						<i class="fa fa-calendar"></i> <?php echo $this->Time->nicedate($post['Post']['modified']); ?> 
						</div>
					
						<div class="white_strip">
						<i class="fa fa-tags"></i> Tags  <?php $string = '';
								foreach($post['Tag'] as $tags){
									$string .= $this->Html->link($tags['name'], array("controller"=>"posts","action"=>"search","by"=>$tags['keyname'],"tid"=>$tags['id']),array("escape"=>false)).", ";
						...
						<div class="white_strip">
							...
							<?php echo $post['Post']['comment_count'];echo (count($post['Post']['comment_count'])>1)?" Comments":" Comment" ;?>
						</div>
					</div>
				</div>
				<p><?php echo preg_replace("/<img /","<img class='img-responsive' ",$post['Post']['body']);?></p>
					...
					<div id="collapseOne" class="panel-collapse collapse">
						<div class="panel-body">
							...
							<div class="row">
								...
								<?php } ?>
								<div class="col-md-1">
									<?php echo $this->Form->input('Comment.post_id',array("value"=>$post_id,"type"=>"hidden","class"=>"form-control ie7-margin")); echo $this->Form->error('Comment.post_id');?>
								</div>
							</div>
							<div class="row">
			...
			<h4><?php echo $post['Post']['comment_count'];echo ($post['Post']['comment_count']>1)?" Comments":" Comment" ;?></h4>
				<?php 
...
<?php echo $this->Html->script('validate',FALSE); ?>		
            ...
            <!-- content goes here -->
			<?php echo $this->Form->create('', array("url"=>array("controller"=>"posts","action"=>"sendtofriend","admin"=>false)));?>
              <div class="row">					
					<div class="form-group col-md-6">
				...
				<div class="row">					
					...
					<?php echo $this->Form->text('Tofriend.post_id',array("type"=>"hidden","class"=>"form-control","value"=>$post['Post']['id'],"id"=>"post_id")); echo $this->Form->error('Tofriend.post_id');echo $this->Form->error('Tofriend.userip');?>
				</div>
				<div class="row">
					<div class="form-group col-md-12">
