    ...
    public function getOptionParser()
    {
        ...
        return parent::getOptionParser()
            ...
            ->addSubcommand('acoUpdate', array(
                ...
                ))->addSubcommand('setSU', array(
                'help' => __('Grant an SuperUser permission to an Group of Users (ARO).'),
                'parser' => array(
                    'description' => array(
                        __('Use this command to grant ACL permissions to AclManager Plugin.')
                    ),
                    'arguments' => array(
                        'aro' => array('help' =>__('User Group ID (ex: Group.1) to grant permission.'),
                        'required' => true
                        ),
                    )
                )));
...
/**
 * Define a SuperUser Group to Access the AclManager Plugin
 *
 * @param string $type The Aro path (Ex: Group.1)
 * @access public
 * @return void
 */
    ...
    {
        $args = $this->args;
        $dispatchReturn = $this->dispatchShell('acl', 'grant', $args[0], 'controllers/AclManager/AclManagers');
        $this->out($dispatchReturn);
    }
}
